        ...
        try {
            ...
            if (event.config.dependencies && typeof event.config.dependencies == "object") {		
				for (const packageName in event.config.dependencies) {
					...
					try {
						...
						logger(getText("notFoundPackage", packageName, event.config.name), "warn");
				...
				logger(getText("loadedPackage", event.config.name),'[ 𝗘𝗩𝗘𝗡𝗧 ]');
			}
            ...
            if (event.config.envConfig && typeof event.config.envConfig == "Object") {
                try {
					...
					logger(getText("loadedConfig", event.config.name),'[ 𝗘𝗩𝗘𝗡𝗧 ]');
				}
			...
			logger(`Đã tải thành công ${event.config.name}!`,'[ 𝗘𝗩𝗘𝗡𝗧 ]');
...
module.exports.unloadModule = function ({ moduleList, threadID, messageID, getText }) {
    ...
    for (const nameModule of moduleList) {
        ...
        logger(getText("unloadSuccess", nameModule),'[ 𝗘𝗩𝗘𝗡𝗧 ]');
    }
